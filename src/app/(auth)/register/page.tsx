...
export default async function Register({ searchParams }: { searchParams: Promise<{error?:string}> }) {
  ...
  return <div><div className="mb-8"><Brand className="mb-8 lg:hidden"/><p className="mb-2 text-sm font-medium text-muted-foreground">Votre carnet relationnel privé</p><h1 className="text-3xl font-semibold tracking-tight">Créer votre espace</h1><p className="mt-2 text-sm text-muted-foreground">Quelques secondes suffisent pour commencer.</p></div>
    {!open&&<p className="mb-4 rounded-lg bg-muted p-3 text-sm text-muted-foreground">Les nouvelles inscriptions sont fermées sur cette instance.</p>}
    {error && open && <p className="mb-4 rounded-lg bg-destructive/10 p-3 text-sm text-destructive">{error === "exists" ? "Cet e-mail existe déjà." : error==="rate-limit"?"Trop de tentatives. Réessayez dans quelques minutes.":"Vérifiez les champs. Le mot de passe doit faire 8 caractères."}</p>}
    {open&&<form action={register} noValidate className="grid gap-4"><FormField label="Votre nom"><TextField name="name" required placeholder="Sacha Guignard"/></FormField><FormField label="Adresse e-mail"><TextField type="email" name="email" required placeholder="vous@exemple.fr"/></FormField><FormField label="Mot de passe" hint="8 caractères minimum"><TextField type="password" name="password" minLength={8} required/></FormField><SubmitButton>Créer mon espace</SubmitButton></form>}
