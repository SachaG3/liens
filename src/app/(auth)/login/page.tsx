...
export default async function Login({ searchParams }: { searchParams: Promise<{error?:string}> }) {
  ...
  return <div><div className="mb-8"><Brand className="mb-8 lg:hidden"/><p className="mb-2 text-sm font-medium text-muted-foreground">Heureux de vous revoir</p><h1 className="text-3xl font-semibold tracking-tight">Connectez-vous</h1><p className="mt-2 text-sm text-muted-foreground">Retrouvez les personnes qui comptent.</p></div>
    {error && <p className="mb-4 rounded-lg bg-destructive/10 p-3 text-sm text-destructive">{error==="rate-limit"?"Trop de tentatives. Réessayez dans quelques minutes.":error==="registration-closed"?"Les nouvelles inscriptions sont fermées.":"Identifiants incorrects."}</p>}
