...
import { Ban, Bell, Cake, CalendarClock, CalendarHeart, Check, Clock3, Landmark, Sparkles } from "lucide-react";
...
import { frenchPublicHolidays, importantDateSuggestions, nextImportantDate } from "@/lib/important-dates";
...
export default async function RemindersPage({searchParams}:{searchParams:Promise<{page?:string}>}) {
  ...
  const [reminders,contacts,importantDates]=await Promise.all([
    ...
    db.contact.findMany({where:{userId:user.id},select:{id:true,firstName:true,lastName:true,photo:true,birthday:true,nameDayReference:true,followUpStatus:true,gender:true,motherId:true,fatherId:true,relationTags:{select:{tag:true}}},orderBy:{firstName:"asc"}}),
  ...
  const dates=importantDates.map(item=>({...item,next:nextImportantDate(item,TODAY)})).filter(item=>item.next>=TODAY&&item.next.getTime()-TODAY.getTime()<=90*86_400_000).sort((a,b)=>a.next.getTime()-b.next.getTime());
  const existingAutomatic=new Set(importantDates.filter(item=>item.sourceKey).map(item=>`${item.contactId}:${item.sourceKey}`));
  ...
  return <Shell><div className="mx-auto max-w-5xl"><header className="mb-9 flex flex-wrap items-end justify-between gap-4"><div><p className="mb-2 text-sm text-muted-foreground">Votre mémoire externe</p><h1 className="text-3xl font-semibold tracking-tight">Rappels et dates</h1><p className="mt-2 text-sm text-muted-foreground">Ce qui mérite votre attention, au bon moment.</p></div><div className="flex gap-2">{suggestions.length>0&&<Modal title="Suggestions automatiques" label="Suggestions" secondary icon={<Sparkles/>} description="Dates calculées depuis les prénoms, votre arbre généalogique et vos relations." wide><ImportantDateSuggestions suggestions={suggestions}/></Modal>}<Modal title="Nouveau rappel" label="Rappel"><ReminderForm contacts={contacts.filter(c=>c.followUpStatus==="active")}/></Modal></div></header>
    ...
    <aside className="space-y-7"><div><div className="mb-4 flex items-center gap-2"><Cake className="size-4"/><h2 className="font-semibold">Anniversaires à venir</h2></div><div className="overflow-hidden rounded-xl border bg-card">{birthdays.map((contact,index)=><Link href={`/contacts/${contact.id}`} key={contact.id} className={`flex items-center gap-3 p-4 transition hover:bg-muted/60 ${index>0?"border-t":""}`}><ProfileAvatar photo={contact.photo} name={contact.firstName+" "+contact.lastName}/><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium">{contact.firstName} {contact.lastName}</p><p className="text-xs text-muted-foreground">{contact.next.toLocaleDateString("fr-FR",{day:"numeric",month:"long"})}</p></div><span className="text-xs text-muted-foreground">{Math.ceil((contact.next.getTime()-TODAY.getTime())/86_400_000)}j</span></Link>)}{!birthdays.length&&<p className="p-6 text-center text-sm text-muted-foreground">Aucun anniversaire dans les 90 prochains jours.</p>}</div></div><div><div className="mb-4 flex items-center gap-2"><CalendarHeart className="size-4"/><h2 className="font-semibold">Dates importantes</h2></div><div className="overflow-hidden rounded-xl border bg-card">{dates.map((item,index)=><Link href={`/contacts/${item.contactId}`} key={item.id} className={`block p-4 transition hover:bg-muted/60 ${index>0?"border-t":""}`}><div className="flex justify-between gap-2"><p className="truncate text-sm font-medium">{item.title}</p><span className="text-xs text-muted-foreground">{Math.ceil((item.next.getTime()-TODAY.getTime())/86_400_000)}j</span></div><p className="mt-1 text-xs text-muted-foreground">{item.contact.firstName} · {item.next.toLocaleDateString("fr-FR",{day:"numeric",month:"long"})}</p></Link>)}{!dates.length&&<p className="p-6 text-center text-sm text-muted-foreground">Aucune date importante à venir.</p>}</div></div><div><div className="mb-4 flex items-center gap-2"><Landmark className="size-4"/><h2 className="font-semibold">Jours fériés français</h2></div><div className="overflow-hidden rounded-xl border bg-card">{holidays.map((item,index)=><div key={item.title} className={`flex justify-between gap-3 p-4 ${index>0?"border-t":""}`}><p className="text-sm font-medium">{item.title}</p><span className="shrink-0 text-xs text-muted-foreground">{item.date.toLocaleDateString("fr-FR",{day:"numeric",month:"short"})}</span></div>)}{!holidays.length&&<p className="p-6 text-center text-sm text-muted-foreground">Aucun jour férié prochainement.</p>}</div></div></aside></div>
    {totalDone>0&&<PaginationControls totalItems={totalDone} itemsPerPage={ITEMS_PER_PAGE} currentPage={currentPage}/>}
  </div></Shell>;
}

function ReminderSection({title,icon,reminders,empty,destructive=false,completed=false}:{title:string;icon:React.ReactNode;reminders:Array<{id:string;kind:string;title:string;dueAt:Date;contactId:string;contact:{firstName:string;lastName:string}}>;empty:string;destructive?:boolean;completed?:boolean}) {
  return <section><div className="mb-3 flex items-center gap-2 text-sm font-semibold [&_svg]:size-4">{icon}<h2>{title}</h2><span className="ml-auto text-xs font-normal text-muted-foreground">{reminders.length}</span></div><div className="overflow-hidden rounded-xl border bg-card">{reminders.map((r,index)=><div key={r.id} className={`flex items-center gap-3 p-3 ${index>0?"border-t":""} ${completed?"opacity-60":""}`}><form action={toggleReminder}><input type="hidden" name="id" value={r.id}/><Button type="submit" variant="outline" size="icon-sm" className="rounded-full"><Check/></Button></form><Link href={`/contacts/${r.contactId}`} className="min-w-0 flex-1"><div className="flex flex-wrap items-center gap-2"><p className={`truncate text-sm font-medium ${completed?"line-through":""}`}>{r.title}</p>{r.kind==="no_contact"&&<span className="flex items-center gap-1 rounded-full bg-rose-500/10 px-2 py-0.5 text-[11px] font-medium text-rose-700 dark:text-rose-300"><Ban className="size-3"/>Ne pas contacter</span>}</div><p className={`text-xs ${destructive?"text-destructive":"text-muted-foreground"}`}>{r.contact.firstName} {r.contact.lastName} · {r.dueAt.toLocaleDateString("fr-FR")}</p></Link>{!completed&&<form action={snoozeReminder}><input type="hidden" name="id" value={r.id}/><Button type="submit" variant="ghost" size="icon-sm" title="Reporter de 7 jours"><CalendarClock/></Button></form>}<Modal title="Modifier le rappel" label="" secondary><EditReminderForm reminder={r}/></Modal><ConfirmDelete action={deleteReminder} id={r.id} message={`Supprimer le rappel « ${r.title} » ?`}/></div>)}{!reminders.length&&<p className="p-6 text-center text-sm text-muted-foreground">{empty}</p>}</div></section>;
}
