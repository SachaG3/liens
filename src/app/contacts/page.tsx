...
export default async function Contacts({searchParams}:{searchParams:Promise<{relation?:string;page?:string;circle?:string;q?:string;sort?:string;status?:string}>}) {
  const user=await requireUser();
  const {relation="all",page="1",circle="all",q="",sort="priority",status="all"}=await searchParams;
  ...
  if(["active","no_contact","deceased"].includes(status))andConditions.push({followUpStatus:status});
  ...
  if(sort==="priority"){
    allPeople.sort((a,b)=>(a.followUpStatus==="active"?0:1)-(b.followUpStatus==="active"?0:1)||a.score-b.score||a.firstName.localeCompare(b.firstName,"fr"));
  }else if(sort==="recent"){
