...
export default async function AccountPage() {
  ...
  const mother=people.find(person=>person.id===user.motherId),father=people.find(person=>person.id===user.fatherId);
  return <Shell><div className="mx-auto max-w-3xl"><header className="mb-8 flex items-center gap-4"><ProfileAvatar photo={user.photo} name={user.name} className="size-16"/><div><p className="mb-1 text-sm text-muted-foreground">Préférences personnelles</p><h1 className="text-3xl font-semibold tracking-tight">Mon compte</h1><p className="mt-1 text-sm text-muted-foreground">Modifiez votre identité de connexion et sécurisez votre accès.</p></div></header>{(mother||father)&&<section className="card mb-6 p-4"><div className="flex flex-wrap items-center gap-2"><span className="mr-2 flex items-center gap-2 text-sm font-medium"><UserRound className="size-4"/>Parents enregistrés</span>{mother&&<ParentLink label="Mère" person={mother}/>} {father&&<ParentLink label="Père" person={father}/>}<Link href="/map" className="ml-auto text-xs font-medium text-muted-foreground hover:text-foreground">Voir l’arbre</Link></div></section>}<div className="grid gap-6 md:grid-cols-2"><section className="card p-6"><h2 className="mb-1 text-xl font-semibold">Profil et généalogie</h2><p className="mb-5 text-sm text-muted-foreground">Définissez vos parents pour construire les branches maternelle et paternelle.</p><AccountForm user={user} people={people}/></section><section className="card p-6"><h2 className="mb-1 text-xl font-semibold">Mot de passe</h2><p className="mb-5 text-sm text-muted-foreground">Vous serez déconnecté après le changement.</p><PasswordForm/></section></div></div></Shell>;
}
