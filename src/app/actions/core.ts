...
import { formDate, formEmail, formPositiveInteger, formPositiveNumber, formText, parseImportedContacts } from "@/lib/action-input";
import { rateLimitRequest } from "@/lib/rate-limit";
import { registrationIsOpen } from "@/lib/registration";
import { ownsAllContacts } from "@/lib/authorization";

const text = formText;

export async function register(form: FormData) {
  if (!registrationIsOpen()) redirect("/login?error=registration-closed");
  if (!(await rateLimitRequest("register", 5, 15 * 60_000))) redirect("/register?error=rate-limit");
  const name = text(form, "name"), emailResult = formEmail(form, "email"), password = text(form, "password");
  if (!name || !emailResult.success || password.length < 8) redirect("/register?error=invalid");
...
export async function login(form: FormData) {
  if (!(await rateLimitRequest("login", 10, 15 * 60_000))) redirect("/login?error=rate-limit");
  const emailResult = formEmail(form, "email"), password = text(form, "password");
  if (!emailResult.success) redirect("/login?error=invalid");
  const email = emailResult.data;
...
export async function updateAccount(form: FormData) {
  ...
  const name=text(form,"name"),emailResult=formEmail(form,"email");
  if(!name||!emailResult.success)return false;
  const email=emailResult.data;
  ...
  const photo=await saveImage(form.get("photo"),user.photo);
  await db.$transaction(async tx=>{
    await tx.user.update({where:{id:user.id},data:{name,email,photo,motherId,fatherId,spouseId}});
    await tx.contactRelationTag.deleteMany({where:{contact:{userId:user.id},tag:{in:["Mère","Père"]}}});
    if(motherId)await tx.contactRelationTag.create({data:{contactId:motherId,tag:"Mère"}});
    if(fatherId)await tx.contactRelationTag.create({data:{contactId:fatherId,tag:"Père"}});
  });
  if(photo!==user.photo)await deleteImage(user.photo);
...
export async function addCircle(form: FormData) {
  ...
  await db.circle.create({ data: {
    ...
    frequency: formPositiveInteger(form, "frequency", 30), weeklyTarget: formPositiveInteger(form, "weeklyTarget", 1),
  }});
  revalidatePath("/");
...
export async function updateCircle(form: FormData) {
  ...
  await db.circle.update({where:{id},data:{name,color:text(form,"color")||circle.color,frequency:formPositiveInteger(form,"frequency",30),weeklyTarget:formPositiveInteger(form,"weeklyTarget",1)}});
...
export async function addContact(form: FormData) {
  ...
  const birthday=formDate(form,"birthday"),deceasedAt=formDate(form,"deceasedAt");
  if(!birthday.success||!deceasedAt.success)return false;
  ...
  const contact = await db.contact.create({ data: {
    ...
    notes: text(form, "notes"), photo: await saveImage(form.get("photo")), desiredFrequency: formPositiveInteger(form,"frequency",30),
    birthday:birthday.data,
    nameDayReference:validNameDayReference(text(form,"nameDayReference")),
    followUpStatus:followUpStatus(text(form,"followUpStatus")),statusNote:text(form,"statusNote"),deceasedAt:deceasedAt.data,
...
export async function updateContact(form: FormData) {
  ...
  const birthday=formDate(form,"birthday"),deceasedAt=formDate(form,"deceasedAt");
  if(!birthday.success||!deceasedAt.success)return false;
  const photo=await saveImage(form.get("photo"),contact.photo);
  await db.contact.update({ where: { id }, data: {
    ...
    notes: text(form, "notes"), photo, desiredFrequency: formPositiveInteger(form,"frequency",30),
    birthday:birthday.data,
    nameDayReference:validNameDayReference(text(form,"nameDayReference")),
    followUpStatus:followUpStatus(text(form,"followUpStatus")),statusNote:text(form,"statusNote"),deceasedAt:deceasedAt.data,
    motherId, fatherId, spouseId, gender:familyGender(text(form,"gender")),
  ...
  if(photo!==contact.photo)await deleteImage(contact.photo);
...
export async function addGroupInteraction(form: FormData) {
  ...
  if(requestedContactIds.length<2||!(await ownsAllContacts(db,user.id,requestedContactIds)))return false;
  const contacts=requestedContactIds.map(id=>({id}));
  const note = text(form, "note");
  const type = text(form, "type") || "meeting";
  const happenedAtResult=formDate(form,"happenedAt");if(!happenedAtResult.success)return false;
  const happenedAt=happenedAtResult.data??new Date();
...
export async function updateInteraction(form:FormData){
  ...
  const note=text(form,"note");
  const happenedAt=formDate(form,"happenedAt");if(!happenedAt.success)return false;
  await db.interaction.update({where:{id},data:{type:text(form,"type")||"message",note,happenedAt:happenedAt.data??interaction.happenedAt}});
...
export async function addJournalEntry(form: FormData) {
  ...
  const happenedAt=formDate(form,"happenedAt");if(!happenedAt.success)return false;
  await db.journalEntry.create({data:{contactId,title,type:text(form,"type")||"note",content:text(form,"content"),happenedAt:happenedAt.data??new Date(),private:form.get("private")==="on"}});
...
export async function updateJournalEntry(form:FormData){
  ...
  const happenedAt=formDate(form,"happenedAt");if(!happenedAt.success)return false;
  await db.journalEntry.update({where:{id},data:{title,type:text(form,"type")||"note",content:text(form,"content"),happenedAt:happenedAt.data??item.happenedAt,private:form.get("private")==="on"}});
...
export async function addImportantDate(form:FormData){
  const user=await requireUser();const contactId=text(form,"contactId"),title=text(form,"title"),date=formDate(form,"date");
  const contact=await db.contact.findFirst({where:{id:contactId,userId:user.id}});if(!contact||!title||!date.success||!date.data)return false;
  await db.importantDate.create({data:{contactId,title,date:date.data,recurring:form.get("recurring")==="on",remindDays:formPositiveInteger(form,"remindDays",7)}});
...
export async function updateImportantDate(form:FormData){
  const user=await requireUser();const id=text(form,"id"),title=text(form,"title"),date=formDate(form,"date");
  const item=await db.importantDate.findFirst({where:{id,contact:{userId:user.id}}});if(!item||!title||!date.success||!date.data)return false;
  await db.importantDate.update({where:{id},data:{title,date:date.data,recurring:form.get("recurring")==="on",remindDays:formPositiveInteger(form,"remindDays",7)}});
...
export async function syncSuggestedImportantDates(form:FormData){
  ...
  await db.$transaction(suggestions.map(item=>db.importantDate.upsert({where:{contactId_sourceKey:{contactId:item.contactId,sourceKey:item.sourceKey}},create:{contactId:item.contactId,source:"automatic",sourceKey:item.sourceKey,title:item.title,date:item.date,recurring:false,remindDays:item.remindDays},update:{title:item.title,date:item.date,recurring:false,remindDays:item.remindDays}})));
...
export async function addContactRelation(form:FormData){
  ...
  if(!(await ownsAllContacts(db,user.id,[sourceId,targetId])))return false;
...
export async function addDebt(form:FormData){
  ...
  const amount=formPositiveNumber(form,"amount"),dueAt=formDate(form,"dueAt");
  const contact=await db.contact.findFirst({where:{id:contactId,userId:user.id}});
  if(!contact||!title||!amount.success||!dueAt.success)return false;
  await db.debt.create({data:{userId:user.id,contactId,title,direction,amount:amount.data,currency:text(form,"currency")||"EUR",note:text(form,"note"),dueAt:dueAt.data}});
  revalidatePath("/finances");revalidatePath(`/contacts/${contactId}`);return true;
}

export async function updateDebt(form:FormData){
  const user=await requireUser();const id=text(form,"id"),title=text(form,"title"),amount=formPositiveNumber(form,"amount"),dueAt=formDate(form,"dueAt");
  const debt=await db.debt.findFirst({where:{id,userId:user.id}});if(!debt||!title||!amount.success||!dueAt.success)return false;
  ...
  await db.debt.update({where:{id},data:{contactId,title,amount:amount.data,direction:text(form,"direction")==="i_owe"?"i_owe":"owed_to_me",currency:text(form,"currency")||"EUR",note:text(form,"note"),dueAt:dueAt.data}});
...
export async function addPet(form:FormData){
  ...
  const birthday=formDate(form,"birthday");if(!birthday.success)return false;
  ...
  await db.pet.create({data:{userId:user.id,name,species:text(form,"species")||"Autre",breed:text(form,"breed"),photo:await saveImage(form.get("photo")),birthday:birthday.data,notes:text(form,"notes"),owners:{create:owners.map(({id:contactId})=>({contactId}))}}});
...
export async function updatePet(form:FormData){
  ...
  const requestedOwnerIds=[...new Set(form.getAll("ownerIds").map(String))];
  const owners=await db.contact.findMany({where:{userId:user.id,id:{in:requestedOwnerIds}},select:{id:true}});
  const photo=await saveImage(form.get("photo"),pet.photo);
  await db.pet.update({where:{id},data:{name,species:text(form,"species")||"Autre",breed:text(form,"breed"),photo,birthday:birthday.data,notes:text(form,"notes"),owners:{deleteMany:{},create:owners.map(({id:contactId})=>({contactId}))}}});
  if(photo!==pet.photo)await deleteImage(pet.photo);
  revalidatePath("/pets");return true;
}
...
export async function addReminder(form: FormData) {
  ...
  const contact = await db.contact.findFirst({ where: { id: contactId, userId: user.id } });
  const dueAt=formDate(form,"dueAt");
  if (!contact || !text(form, "title") || !dueAt.success || !dueAt.data || (kind==="contact"&&contact.followUpStatus!=="active")) return false;
  await db.reminder.create({ data: { contactId, kind, title: text(form, "title"), dueAt:dueAt.data } });
  revalidatePath("/");
...
export async function updateReminder(form: FormData) {
  ...
  const dueAt = formDate(form, "dueAt");
  if (!title || !dueAt.success || !dueAt.data) return false;
  await db.reminder.update({ where: { id }, data: { kind: reminderKind(text(form, "kind")), title, dueAt:dueAt.data } });
  revalidatePath("/");
  revalidatePath("/reminders");
...
export async function addGiftIdea(form: FormData) {
  ...
  const price=rawPrice?formPositiveNumber(form,"price"):null;
  if(price&&!price.success)return false;
  await db.giftIdea.create({ data: { contactId, title, url: text(form, "url"), note: text(form, "note"), price: price?.data??null } });
  revalidatePath(`/contacts/${contactId}`);
...
export async function updateGiftIdea(form:FormData){
  ...
  const rawPrice=text(form,"price").replace(",",".");
  const price=rawPrice?formPositiveNumber(form,"price"):null;if(price&&!price.success)return false;
  await db.giftIdea.update({where:{id},data:{title,url:text(form,"url"),note:text(form,"note"),price:price?.data??null}});
...
export async function importContacts(form: FormData) {
  const user = await requireUser();
  const parsed=parseImportedContacts(text(form,"contacts"));
  if(!parsed)return;
  ...
  await db.$transaction(async tx=>{for (const contact of contacts) {
    ...
    if(choice.action==="merge"&&choice.targetId){
      const existing=await tx.contact.findFirst({where:{id:choice.targetId,userId:user.id}});
      if(!existing)continue;
      await tx.contact.update({where:{id:existing.id},data:{firstName:existing.firstName||data.firstName,lastName:existing.lastName||data.lastName,email:existing.email||data.email,phone:existing.phone||data.phone,company:existing.company||data.company,notes:existing.notes||data.notes,birthday:existing.birthday||data.birthday,sourceId:existing.sourceId||data.sourceId}});
      for(const circleId of circleIds)await tx.circleMember.upsert({where:{contactId_circleId:{contactId:existing.id,circleId}},create:{contactId:existing.id,circleId},update:{}});
      for(const tag of relationTags)await tx.contactRelationTag.upsert({where:{contactId_tag:{contactId:existing.id,tag}},create:{contactId:existing.id,tag},update:{}});
    }else{
      await tx.contact.create({data:{...data,userId:user.id,circles:{create:circleIds.map(circleId=>({circleId}))},relationTags:{create:relationTags.map(tag=>({tag}))}}});
    }
