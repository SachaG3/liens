...
export default async function Dashboard() {
  ...
  const scored=contacts.filter(c=>!c.reminders.length).map(c=>({...c,score:relationshipScore(c.interactions[0]?.happenedAt??null,effectiveFrequency(c.desiredFrequency,c.circles.map(({circle})=>circle.frequency)))})).sort((a,b)=>a.score-b.score);
