...
import { ContactForm, GroupInteractionForm, InteractionForm } from "@/components/forms";
...
export default async function Dashboard() {
  ...
  return <Shell><div className="mx-auto max-w-6xl">
    <header className="mb-10 flex flex-wrap items-end justify-between gap-4"><div><p className="mb-2 text-sm capitalize text-muted-foreground">{date}</p><h1 className="text-3xl font-semibold tracking-tight">Bonjour {user.name.split(" ")[0]}.</h1><p className="mt-2 text-muted-foreground">{scored.filter(c=>c.score<45).length?`${scored.filter(c=>c.score<45).length} personnes méritent un petit message.`:"Vos relations importantes sont à jour."}</p></div><div className="flex flex-wrap gap-2"><Modal title="Enregistrer un échange" label="Échange" secondary><InteractionForm contacts={contacts.filter(c=>c.followUpStatus==="active")}/></Modal><Modal title="Enregistrer un événement de groupe" label="Événement de groupe" secondary wide description="Créez le même échange dans l’historique de chaque personne présente."><GroupInteractionForm contacts={contacts.filter(c=>c.followUpStatus==="active")} otherContacts={contacts.filter(c=>c.followUpStatus!=="active")}/></Modal><Modal title="Ajouter une personne" label="Personne" wide><ContactForm circles={circles} people={contacts}/></Modal></div></header>
