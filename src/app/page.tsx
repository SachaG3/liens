...
import { ArrowRight, Check, MessageCircle, Plus, Users } from "lucide-react";
...
export default async function Dashboard() {
  ...
  const [contacts,circles,reminders,interactionCount]=await Promise.all([
    db.contact.findMany({where:{userId:user.id},include:{relationTags:true,interactions:{orderBy:{happenedAt:"desc"},take:1},circles:{include:{circle:true}}},orderBy:{updatedAt:"desc"}}),
    ...
    db.reminder.findMany({where:{contact:{userId:user.id},done:false},include:{contact:true},orderBy:{dueAt:"asc"},take:5}),
    db.interaction.count({where:{contact:{userId:user.id},happenedAt:{gte:WEEK_AGO}}}),
  ]);
  const scored=contacts.map(c=>({...c,score:relationshipScore(c.interactions[0]?.happenedAt??null,effectiveFrequency(c.desiredFrequency,c.circles.map(({circle})=>circle.frequency)))})).sort((a,b)=>a.score-b.score);
