...
import { ContactRelationForm, ConversationItemForm, CustomFieldForm, EditConversationItemForm, EditCustomFieldForm, EditImportantDateForm, EditJournalEntryForm, ImportantDateForm, JournalEntryForm } from "@/components/forms";
...
export function ContactWorkspace({contact,people}:{contact:WorkspaceContact;people:Person[]}) {
  ...
  return <div className="mt-6 grid gap-6 lg:grid-cols-[1.25fr_.9fr]">
    ...
    <div className="space-y-6">
      <CompactPanel icon={<CalendarHeart/>} title="Dates importantes" count={contact.importantDates.length} modal={<Modal title="Nouvelle date importante" label="" secondary><ImportantDateForm contactId={contact.id}/></Modal>}>{contact.importantDates.slice(0,4).map(item=><div key={item.id} className="flex items-center gap-1 text-sm"><span className="min-w-0 flex-1 truncate">{item.title}</span><span className="shrink-0 text-muted-foreground">{item.date.toLocaleDateString("fr-FR",{day:"numeric",month:"short"})}</span><Modal title="Modifier la date" label="" secondary><EditImportantDateForm item={item}/></Modal><ConfirmDelete action={deleteImportantDate} id={item.id} message={`Supprimer « ${item.title} » ?`}/></div>)}</CompactPanel>
      <CompactPanel icon={<Link2/>} title="Relations entre personnes" count={relations.length} modal={<Modal title="Lier deux personnes" label="" secondary><ContactRelationForm contactId={contact.id} people={people}/></Modal>}>{relations.slice(0,4).map(item=><div key={item.id} className="flex items-center gap-1 text-sm"><Link href={`/contacts/${item.person.id}`} className="min-w-0 flex-1 truncate hover:underline">{item.person.firstName} {item.person.lastName}</Link><span className="shrink-0 text-muted-foreground">{item.label||"Lié"}</span><ConfirmDelete action={deleteContactRelation} id={item.id} message={`Retirer le lien avec ${item.person.firstName} ?`}/></div>)}</CompactPanel>
      <CompactPanel icon={<FileText/>} title="Journal universel" count={contact.journalEntries.length} modal={<Modal title="Ajouter au journal" label="" secondary><JournalEntryForm contactId={contact.id}/></Modal>}>{contact.journalEntries.slice(0,3).map(item=><div key={item.id} className="flex items-start gap-1 text-sm"><div className="min-w-0 flex-1">{item.private?<SensitiveValue value={item.title}/>:<span className="truncate font-medium">{item.title}</span>}<p className="text-xs text-muted-foreground">{item.happenedAt.toLocaleDateString("fr-FR")}</p></div><Modal title="Modifier l’entrée" label="" secondary><EditJournalEntryForm item={item}/></Modal><ConfirmDelete action={deleteJournalEntry} id={item.id} message={`Supprimer « ${item.title} » ?`}/></div>)}</CompactPanel>
      <CompactPanel icon={<ListTodo/>} title="Champs personnalisés" count={contact.customFields.length} modal={<Modal title="Nouveau champ personnalisé" label="" secondary><CustomFieldForm contactId={contact.id}/></Modal>}>{contact.customFields.slice(0,4).map(item=><div key={item.id} className="flex items-center gap-1 text-sm"><span className="min-w-0 flex-1 truncate text-muted-foreground">{item.label}</span>{item.private?<SensitiveValue value={item.value}/>:<span className="max-w-40 truncate font-medium">{item.value}</span>}<Modal title="Modifier le champ" label="" secondary><EditCustomFieldForm item={item}/></Modal><ConfirmDelete action={deleteCustomField} id={item.id} message={`Supprimer le champ « ${item.label} » ?`}/></div>)}</CompactPanel>
    </div>
  </div>;
}
