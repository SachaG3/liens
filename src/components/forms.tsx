...
import { nameDayOptions } from "@/lib/important-dates";
...
export function CircleForm() {
  return <ModalForm action={addCircle} noValidate className="grid gap-4">
    ...
    <div className="grid gap-3 sm:grid-cols-2"><FormField label="Couleur"><TextField className="p-1" type="color" name="color" defaultValue="#6655e8"/></FormField><FormField label="Contact tous les…"><TextField type="number" name="frequency" defaultValue="30" min="1"/></FormField></div>
...
export function EditCircleForm({circle}:{circle:{id:string;name:string;color:string;frequency:number;weeklyTarget:number}}) {
  return <ModalForm action={updateCircle} noValidate className="grid gap-4"><input type="hidden" name="id" value={circle.id}/>
    ...
    <div className="grid gap-3 sm:grid-cols-2"><FormField label="Couleur"><TextField className="p-1" type="color" name="color" defaultValue={circle.color}/></FormField><FormField label="Contact tous les…"><TextField type="number" name="frequency" defaultValue={circle.frequency} min="1"/></FormField></div>
...
export function ContactForm({ circles, people = [] }: { circles:Array<{id:string;name:string;color:string}>;people?:MentionPerson[] }) {
  return <ModalForm action={addContact} noValidate><FormSteps submitLabel="Ajouter la personne" steps={[
    {label:"Identité",description:"Les informations essentielles et les coordonnées.",content:<><PhotoEditor/><div className="grid gap-3 sm:grid-cols-2"><FormField label="Prénom"><TextField name="firstName" required/></FormField><FormField label="Nom"><TextField name="lastName"/></FormField></div><div className="grid gap-3 sm:grid-cols-2"><FormField label="E-mail"><TextField type="email" name="email"/></FormField><FormField label="Téléphone"><TextField name="phone"/></FormField></div><FormField label="Entreprise"><TextField name="company"/></FormField><FormField label="Anniversaire"><TextField type="date" name="birthday"/></FormField><NameDayReferenceField/></>},
...
export function EditContactForm({ contact, circles, people }: {
  ...
  return <ModalForm action={updateContact} noValidate><input type="hidden" name="id" value={contact.id}/><FormSteps submitLabel="Enregistrer" steps={[
    {label:"Identité",description:"Photo, identité et coordonnées.",content:<><PhotoEditor existingPhoto={contact.photo}/><div className="grid gap-3 sm:grid-cols-2"><FormField label="Prénom"><TextField name="firstName" required defaultValue={contact.firstName}/></FormField><FormField label="Nom"><TextField name="lastName" defaultValue={contact.lastName}/></FormField></div><div className="grid gap-3 sm:grid-cols-2"><FormField label="E-mail"><TextField type="email" name="email" defaultValue={contact.email}/></FormField><FormField label="Téléphone"><TextField name="phone" defaultValue={contact.phone}/></FormField></div><FormField label="Entreprise"><TextField name="company" defaultValue={contact.company}/></FormField><FormField label="Anniversaire"><TextField type="date" name="birthday" defaultValue={contact.birthday?.toISOString().slice(0,10)}/></FormField><NameDayReferenceField value={contact.nameDayReference}/></>},
...
export function GroupInteractionForm({contacts,otherContacts=[]}:{contacts:MentionPerson[];otherContacts?:MentionPerson[]}) {
  ...
  return <ModalForm action={addGroupInteraction} noValidate className="grid gap-4">
    ...
    <div className="grid gap-3 sm:grid-cols-2"><FormField label="Type"><NativeSelect name="type" defaultValue="meeting"><option value="meeting">Rencontre</option><option value="call">Appel</option><option value="message">Message</option><option value="email">E-mail</option></NativeSelect></FormField><FormField label="Date"><TextField type="date" name="happenedAt" defaultValue={new Date().toISOString().slice(0,10)}/></FormField></div>
...
export function GiftIdeaForm({ contactId }: { contactId:string }) {
  return <ModalForm action={addGiftIdea} noValidate className="grid gap-4"><input type="hidden" name="contactId" value={contactId}/>
    ...
    <div className="grid gap-3 sm:grid-cols-[1fr_120px]"><FormField label="Lien"><TextField type="url" name="url" placeholder="https://…"/></FormField><FormField label="Budget (€)"><TextField type="number" step="0.01" min="0" name="price"/></FormField></div>
...
export function EditGiftIdeaForm({gift}:{gift:{id:string;title:string;url:string;price:number|null;note:string}}) {
  return <ModalForm action={updateGiftIdea} noValidate className="grid gap-4"><input type="hidden" name="id" value={gift.id}/><FormField label="Idée"><TextField name="title" required defaultValue={gift.title}/></FormField><div className="grid gap-3 sm:grid-cols-[1fr_120px]"><FormField label="Lien"><TextField type="url" name="url" defaultValue={gift.url}/></FormField><FormField label="Budget (€)"><TextField type="number" step="0.01" min="0" name="price" defaultValue={gift.price??undefined}/></FormField></div><FormField label="Note"><TextAreaField name="note" rows={2} defaultValue={gift.note}/></FormField><SubmitButton>Enregistrer l’idée</SubmitButton></ModalForm>;
}

export function JournalEntryForm({contactId}:{contactId:string}) {
  return <ModalForm action={addJournalEntry} className="grid gap-4"><input type="hidden" name="contactId" value={contactId}/><FormField label="Titre"><TextField name="title" required placeholder="Voyage à Lyon, changement de travail…"/></FormField><div className="grid gap-3 sm:grid-cols-2"><FormField label="Type"><NativeSelect name="type"><option value="note">Note</option><option value="event">Événement</option><option value="milestone">Étape importante</option><option value="conflict">Conflit</option><option value="memory">Souvenir</option></NativeSelect></FormField><FormField label="Date"><TextField type="date" name="happenedAt"/></FormField></div><FormField label="Détails"><TextAreaField name="content" rows={4}/></FormField><PrivateToggle/><SubmitButton>Ajouter au journal</SubmitButton></ModalForm>;
}
...
export function DebtForm({people}:{people:MentionPerson[]}) {
  return <ModalForm action={addDebt} className="grid gap-4">
    ...
    <div className="grid gap-3 sm:grid-cols-[1fr_110px]"><FormField label="Montant"><TextField type="number" name="amount" min="0.01" step="0.01" required/></FormField><FormField label="Devise"><NativeSelect name="currency"><option value="EUR">EUR</option><option value="USD">USD</option><option value="GBP">GBP</option><option value="CHF">CHF</option></NativeSelect></FormField></div>
