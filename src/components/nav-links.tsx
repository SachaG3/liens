...
export function NavLinks({mobile=false}:{mobile?:boolean}) {
  ...
  return <nav className="grid grid-cols-4 gap-1">{primary.map(({href,label,icon:Icon})=><Link key={href} href={href} className={cn("flex h-12 flex-col items-center justify-center gap-1 rounded-lg text-[10px] font-medium text-muted-foreground transition-colors",active(href)&&"bg-foreground text-background")}><Icon className="size-4"/><span>{label}</span></Link>)}<details className="group relative"><summary className={cn("flex h-12 cursor-pointer list-none flex-col items-center justify-center gap-1 rounded-lg text-[10px] font-medium text-muted-foreground [&::-webkit-details-marker]:hidden",secondaryActive&&"bg-foreground text-background")}><MoreHorizontal className="size-4"/><span>Plus</span></summary><div className="absolute bottom-14 right-0 grid w-[min(22rem,calc(100vw-1.5rem))] grid-cols-2 gap-1 rounded-xl border bg-card p-2 shadow-xl">{secondary.map(({href,label,icon:Icon})=><Link key={href} href={href} className={cn("flex items-center gap-2 rounded-lg px-3 py-3 text-sm font-medium text-muted-foreground hover:bg-muted hover:text-foreground",active(href)&&"bg-foreground text-background")}><Icon className="size-4"/>{label}</Link>)}</div></details></nav>;
}
